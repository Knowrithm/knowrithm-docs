...
<Columns cols={2}>
  <Card title="Flask API" icon="server">
    **Python 3.11+**. Handles synchronous reads, queues write-heavy operations, issues JWT tokens, and exposes a rich REST surface under `/api/v1`.
  </Card>
  <Card title="Celery & task routing" icon="bolt">
    **Celery 5+**. The shared dispatcher pushes work to priority queues (`critical`, `default`, `low`, `slow`, `dead_letter`). Workers can be scaled horizontally per queue.
  </Card>
  <Card title="Redis" icon="cloud">
    **Redis 6+**. Serves as the Celery broker, cache layer, and transient store for live conversation streaming.
  </Card>
  <Card title="PostgreSQL" icon="layer-group">
    **PostgreSQL 13+**. Stores multi-tenant domain models with **SQLAlchemy ORM**. Handles row-level relationships and soft-delete flags.
  </Card>
</Columns>
...
<AccordionGroup>
  ...
  <Accordion icon="gear" title="LLM settings as first-class entities">
...
<Columns cols={2}>
  <Card title="Web tier" icon="globe">
  ...
  <Card title="Workers" icon="terminal">
  ...
  <Card title="Storage & networking" icon="cubes">
